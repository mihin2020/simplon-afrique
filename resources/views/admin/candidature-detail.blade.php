...
@section('title', 'Détail du dossier - Simplon Africa')
    ...
    <div>
        ...
        <div class="space-y-6">
            ...
            <div class="bg-white rounded-xl shadow-sm p-6">
                <div class="flex items-center justify-between mb-4">
                    ...
                    @if($isSuperAdmin && $candidature->status === 'submitted')
                        <form action="{{ route('admin.candidature.validate', $candidature->id) }}" method="POST" class="inline">
                            @csrf
                            <button
                                type="submit"
                                ...
                                onclick="this.disabled=true; this.innerHTML='<svg class=\'animate-spin h-5 w-5\' fill=\'none\' viewBox=\'0 0 24 24\'><circle class=\'opacity-25\' cx=\'12\' cy=\'12\' r=\'10\' stroke=\'currentColor\' stroke-width=\'4\'></circle><path class=\'opacity-75\' fill=\'currentColor\' d=\'M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z\'></path></svg> Validation en cours...'; this.form.submit();"
                            >
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="bg-white rounded-xl shadow-sm p-6">
                <div class="flex items-center justify-between mb-4">
                    <div>
                        ...
                        <p class="text-xs text-gray-500 mt-1">
                            ...
                            @elseif(!$isSuperAdmin)
                                <span class="text-amber-600 font-medium">⚠️ Seul le super administrateur peut assigner un jury à une candidature</span>
                            @else
                ...
                <div class="space-y-4">
                    @if($canAssignJury)
                        <form action="{{ route('admin.candidature.assign-jury', $candidature->id) }}" method="POST" class="space-y-4">
                            @csrf
                            <div>
                                <label for="jury_id" class="block text-sm font-medium text-gray-700 mb-2">
                                ...
                                >
                                    ...
                                    @foreach($availableJuries as $jury)
                                        @php
                                            ...
                                            $isSelected = $candidature->juries->contains('id', $jury->id);
                                        @endphp
                                        <option value="{{ $jury->id }}" {{ $isSelected ? 'selected' : '' }}>
                                            {{ $jury->name }}
                    ...
                    @if($candidature->juries->isNotEmpty() && $isSuperAdmin)
                        <div class="mt-4">
                            <form action="{{ route('admin.candidature.assign-jury', $candidature->id) }}" method="POST" class="inline">
                                @csrf
                                <input type="hidden" name="jury_id" value="">
                                <button
                                    type="submit"
                                    onclick="return confirm('Êtes-vous sûr de vouloir retirer ce jury de cette candidature ?')"
                                    class="text-sm text-red-600 hover:text-red-700 font-medium flex items-center gap-1.5 px-3 py-1.5 rounded-lg hover:bg-red-50 transition"
                                >
